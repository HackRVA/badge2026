...
void irq_fifo_handler(void) {
    ...
    while (!pio_sm_is_rx_fifo_empty(BADGE_PIO_IR, rx_sm)) {
        uint32_t rx_data = pio_sm_get(BADGE_PIO_IR, rx_sm);
...
void ir_init(void) {
    rx_sm = nec_rx_init(BADGE_PIO_IR, BADGE_GPIO_IR_RX);
    tx_sm = nec_tx_init(BADGE_PIO_IR, BADGE_GPIO_IR_TX);
    ...
    pio_set_irq1_source_enabled(BADGE_PIO_IR, irq_source, true);
...
bool ir_transmitting(void) {
    return pio_sm_is_tx_fifo_empty(BADGE_PIO_IR, tx_sm);
}
...
static void ir_send_start_packet(const IR_DATA *data) {
    ...
    pio_sm_put_blocking(BADGE_PIO_IR, tx_sm, get_raw_nec_data(packet_data));
...
void ir_send_complete_message(const IR_DATA *data) {
    ...
    enum pio_interrupt_source irq_source = pis_sm0_rx_fifo_not_empty + rx_sm;
    pio_set_irq1_source_enabled(BADGE_PIO_IR, irq_source, false);
    // Pause receive while transmitting
    pio_sm_set_enabled(BADGE_PIO_IR, rx_sm, false);
    ...
    pio_sm_put_blocking(BADGE_PIO_IR, tx_sm, 0xa55aa55a);
    ...
    pio_sm_clear_fifos(BADGE_PIO_IR, rx_sm);
    irq_set_enabled(PIO0_IRQ_1, true);
    pio_set_irq1_source_enabled(BADGE_PIO_IR, irq_source, true);
    pio_sm_set_enabled(BADGE_PIO_IR, rx_sm, true);
}
...
uint8_t ir_send_partial_message(const IR_DATA *data, uint8_t starting_sequence_num) {
    ...
    for (int i=starting_sequence_num; i<data->data_length; i++) {
        if (pio_sm_is_tx_fifo_full(BADGE_PIO_IR, tx_sm)) {
