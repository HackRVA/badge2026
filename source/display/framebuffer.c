...
void FbRoundedRect(unsigned char width, unsigned char height, unsigned char stroke)
{
    ...
    for (int i = 0; i < stroke; i++) {
        FbHorizontalLine(x_min + stroke, y_min + i, x_max - stroke, y_min + i);
        FbHorizontalLine(x_min + stroke, y_max - i, x_max - stroke, y_max - i);
    }

    for (int i = 0; i < stroke; i++) {
        FbVerticalLine(x_min + i, y_min + stroke, x_min + i, y_max - stroke);
        FbVerticalLine(x_max - i, y_min + stroke, x_max - i, y_max - stroke);
    }
