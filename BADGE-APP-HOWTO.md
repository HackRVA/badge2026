...
C programmers are encouraged to give it a try.
 ...
 <!--
table of contents generated by https://github.com/ekalinin/github-markdown-toc
 -->
   * [Overview](#overview)
   * [Badge App Template](#badge-app-template)
   * [Drawing on the Screen](#drawing-on-the-screen)
   * [Initializing and Clearing the Framebuffer](#initializing-and-clearing-the-framebuffer)
   * [Updating the Screen](#updating-the-screen)
   * [Colors](#colors)
   * [Drawing Lines and Points and other things](#drawing-lines-and-points-and-other-things)
   * [Moving the "Cursor"](#moving-the-cursor)
   * [Writing Strings to the Framebuffer](#writing-strings-to-the-framebuffer)
   * [Particle system](#particle-system)
   * [Buttons, Directional-Pad Inputs and Rotary Encoders](#buttons-directional-pad-inputs-and-rotary-encoders)
   * [Display Related Functions](#display-related-functions)
   * [Infrared Transmitter and Receiver](#infrared-transmitter-and-receiver)
   * [Transmitting IR](#transmitting-ir)
   * [Receiving IR Packets](#receiving-ir-packets)
   * [Badge ID and User Name:](#badge-id-and-user-name)
   * [Audio](#audio)
   * [Music](#music)
   * [Microphone](#microphone)
   * [Setting the Flair LED Color](#setting-the-flair-led-color)
   * [Sensors](#sensors)
   * [Flash Memory Access](#flash-memory-access)
   * [Exiting the Badge App:](#exiting-the-badge-app)
   * [Miscellaneous Functions:](#miscellaneous-functions)
   * [(Pseudo)random number generators (PRNGs)](#pseudorandom-number-generators-prngs)
   * [Menus](#menus)
   * [Pathfinding](#pathfinding)
   * [Ray Casting](#ray-casting)
   * [Some Common Video Game Programming Patterns](#some-common-video-game-programming-patterns)
   * [Fixed Point Math](#fixed-point-math)
   * [Arrays of Things](#arrays-of-things)
   * [Adding New Objects Into the Array](#adding-new-objects-into-the-array)
   * [Removing Objects from the Array](#removing-objects-from-the-array)
   * [Arrays of Different Types of Things](#arrays-of-different-types-of-things)
   * [Collision detection](#collision-detection)
   * [A few miscellaneous debugging tips](#a-few-miscellaneous-debugging-tips)
   * [A few words about using git](#a-few-words-about-using-git)

Overview
--------

The badge firmware, which includes all the badge apps, is written in C.
If you are new to C, you may find it challenging. If you already know some
other language, such as Java, Python, or C#, you may find [Beej's Guide to
C programming](https://beej.us/guide/bgc/html/split/index.html) helpful in your
journey to learn C. If you're entirely new to computer programming in any language,
the book "C Programming: A Modern Approach" by K. N. King is highly regarded
and suitable for programming neophytes.  The book "The C Programming Language"
by Brian Kernighan and Dennis Ritchie is the venerated classic book on C but it
is not well suited for beginners.
...
* If you are just making changes to your own badge app, it should be pretty safe to just
  push those changes to the main branch. Worst case, if you break the build, your app can
  be commented out easily. You don't need to make a PR for every change.
* It's a good idea to build the firmware as well as the simulator from
  time to time just to be sure you haven't accidentally broken it in some way that doesn't
  show up in the simulator. Ideally you'd want to do this before you've pushed breaking
  changes to main.
* Depending on your level of experience and confidence, you may wish to submit your change
  as a PR to get some review, especially if you are changing something that others rely
  upon and you have some uncertainty about your code.
* It is generally better to cherry-pick changes from your branch onto main, or to rebase
  your branch so that you can fast-forward merge to main in order to keep the history on
  main linear and git-bisectable and free of merge commits.
* It should go without saying that whatever you push to main should build and run and
  ideally be bug free (but of course the last point is impossible to attain 100% of
  the time).
* Each commit should ideally contain one logical change. Avoid co-mingling unrelated
  changes into a single commit.
* In general, you are free to push directly to the main branch, but please be careful.
  Never force push to main.
* You might find [stgit](https://stacked-git.github.io/) to be helpful for making your
  commit history nice and clean and to appear be the work of someone that actually knows
  what they are doing, even if sometimes you actually don't.
* A good commit message should look something like this:

```
        subsystem: imperative description of change

        A concise (but not excessively concise) paragraph describing
	the changes you made to the code and why they were made.
```

  where "subsystem" is the name of your badge app, or the name of whatever subsystem of
  the badge code your change touches. A commit message like "more updates" is not cool.

  A more concrete example of a commit changing the "badgey" badge app:

```
    badgey: fix bad ifdef around errno include

    ifdef __linux__ for that purpose is not cool, as we have Mac users.
