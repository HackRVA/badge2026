# Software

| App | Description |
|-----|-------------|
| **2048** | Puzzle game in which you slide tiles to combine identical tiles to sum to 2048 |
| **AA Gunner** | An anti-aircraft gunner game in which you defend Ukranian cities from missile attacks |
| **Asteroids** | A space-themed multidirectional shooter arcade game. It's similar to the classic asteroids experience. |
| **Badge Monsters** | A collectible monster game. |
| **Batping** | A flappybird-esque game |
| **BattleZone** | A first-person 3D tank combat simulation. The objective is to destroy enemy tanks for points, while avoiding being shot. |
| **Blackjack** | It's BlackJack. |
| **Clue** | A variant of the Clue board game |
| **Daywalker** | An auto-attack survivor game (inspired by vampire survivor) |
| **Digipick** | A lockpicking game similar to lock picking in Starfield |
| **Drum Machine** | A drum machine allowing you to create patterns of drum beats which can be combined into songs |
| **Game of Life** | A zero-player game that simulates cellular automaton, originally designed by British mathematician John Horton Conway. |
| **Lunar Rescue** | A game where the player maneuvers a spaceship to rescue astronauts stranded on the Moon while avoiding obstacles. |
| **MicroBan** | A puzzle game |
| **MixTape** | A Chiptune tape deck simulator |
| **Moon Patrol** | An re-implementation of the 80's Moon Patrol arcade game |
| **Puzzle Attack** | A puzzle game |
| **RVASec Quest** | A fantasy role playing game in which you pursue a quest to attend RVASec (inspired by Ultima III) [See Map](/assets/ossaria-map.jpg)|
| **Screamo** | Test your screaming abilities |
| **Skygolf** | Golf, in the sky (based on a crisp-game-lib demo) |
| **Simon Says** | A reimplementation of the old Simon electronic game |
| **Slot Machine** | A digital simulation of a casino-style slot machine game. The player pulls the lever and hopes for a lucky combination. |
| **Smashout** | A breakout-like game. The player controls a paddle and the objective is to break bricks by bouncing a ball against them. |


### Write your own game
Check out our github repository and follow the instructions to write your own game.
[https://github.com/hackrva/badge2026](https://github.com/hackrva/badge2026)
