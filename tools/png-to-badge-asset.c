...
static int nostatic = 0;
static int skipargs = 0;
char *static_keyword = "static ";

static void __attribute__((noreturn)) usage(char *program_name)
{
	char *p = program_name ? program_name : "png-to-badge-asset";

	fprintf(stderr, "%s: usage: %s [--nostatic] bits-per-pixel png-file(s)\n", p, p);
...
static int generate_c_code(char *filename, char *prefix, int width, int height)
{
	...
	if (bits_per_pixel != 16 && bits_per_pixel != 1) {
		if (nostatic)
			printf("#ifdef DEFINE_IMAGE_ASSET_DATA\n");
	...
	printf("\n");
	printf("%sconst struct asset2 %s%s = {\n", static_keyword, prefix, image_name);
	...
	if (nostatic) {
		printf("#else\n");
		printf("extern const struct asset2 %s%s;\n", prefix, image_name);
		printf("#endif\n");
	}
...
int main(int argc, char *argv[])
{
	...
	int rc = sscanf(argv[1 + skipargs], "%d", &bits_per_pixel);
	if (rc != 1) {
		usage(argv[0]);
		__builtin_unreachable();
	}
